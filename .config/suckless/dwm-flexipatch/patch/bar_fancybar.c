int width_fancybar(Bar *bar, BarArg *a) { return a->w; }
...
int draw_fancybar(Bar *bar, BarArg *a) {
  ...
  if (n > 0) {
    ...
    for (c = m->clients; c; c = c->next) {
      ...
      tw = tabw;
      drw_setscheme(drw,
                    scheme[m->sel == c ? SchemeTitleSel : SchemeTitleNorm]);
...
int click_fancybar(Bar *bar, Arg *arg, BarArg *a) { return ClkWinTitle; }
